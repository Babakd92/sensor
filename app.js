...
const dashboardUpdatedAt = "2026-09-13 06:41";
const ndviImageUrl = "sentinel2_ndvi_latest.png?v=20260913064155";
const ndviAcquiredAt = "2026-09-10";
const ndviCloudPercent = 76.0;
const ndviStatusMessage = "";
const ndviStats = {"mean": 0.779, "min": 0.437, "max": 0.899, "stdDev": 0.084, "lowVegetationPct": 0.0, "moderateVegetationPct": 4.7, "highVegetationPct": 95.3, "interpretation": "Most valid field pixels show dense, vigorous green vegetation."};
...
function shadeHistoryForecast(ctx, width, height, pad, totalCount, historyCount) {
  if (!totalCount || !historyCount) return;
  const plotWidth = width - pad.left - pad.right;
  const plotHeight = height - pad.top - pad.bottom;
  const splitX = pad.left + (plotWidth * historyCount) / totalCount;

  ctx.save();
  ctx.fillStyle = "rgba(168, 50, 50, 0.08)";
  ctx.fillRect(pad.left, pad.top, Math.max(0, splitX - pad.left), plotHeight);
  ctx.fillStyle = "rgba(40, 96, 168, 0.08)";
  ctx.fillRect(splitX, pad.top, Math.max(0, width - pad.right - splitX), plotHeight);
  ctx.restore();
}
...
function drawPrecipChart() {
  ...
  const historyCount = chartData.filter((item) => item.type === "past").length;

  shadeHistoryForecast(ctx, width, height, pad, chartData.length, historyCount);

  drawLegend(ctx, [
    { label: "Historical", color: "#a83232", kind: "bar" },
    { label: "Forecast", color: "#2860a8", kind: "bar" }
  ...
  chartData.forEach((item, index) => {
    ...
    ctx.fillStyle = item.type === "past" ? "#a83232" : "#2860a8";
...
function drawTempChart() {
  ...
  const plotWidth = width - pad.left - pad.right;
  const plotHeight = height - pad.top - pad.bottom;
  const historyCount = chartData.filter((item) => (
  ...
  drawLegend(ctx, [
    { label: "Historical min", color: "#a83232", dashed: true },
    { label: "Historical max", color: "#a83232", dashed: false },
    { label: "Future min", color: "#2860a8", dashed: true },
    { label: "Future max", color: "#2860a8", dashed: false }
  ], pad.left, 24, plotWidth);
  ...
  drawLine(ctx, pastMin, "#a83232", true);
  drawLine(ctx, pastMax, "#a83232", false);
  drawLine(ctx, futureMin, "#2860a8", true);
  drawLine(ctx, futureMax, "#2860a8", false);
...
function renderDataTables() {
  ...
  if (ndviBody) {
    ...
    ndviBody.innerHTML = ndviStats && Object.keys(ndviStats).length
      ...
      `).join("")
      : '<tr><td colspan="3">Run weekly_runner_v4 to load NDVI statistics.</td></tr>';
  }
}
...
function setupExpandableCharts() {
  const cards = Array.from(document.querySelectorAll(".chart-card"));
  cards.forEach((card) => {
    card.addEventListener("click", (event) => {
      if (event.target.closest("button, a, input, select, textarea")) return;
      event.stopPropagation();
      const alreadyExpanded = card.classList.contains("chart-card-expanded");
      cards.forEach((item) => item.classList.remove("chart-card-expanded"));
      if (!alreadyExpanded) {
  ...
  document.addEventListener("click", () => {
    ...
    if (!expanded) return;
    expanded.classList.remove("chart-card-expanded");
    requestAnimationFrame(redrawAllCharts);
  });
}
...
function renderNdviPanel() {
  ...
  if (ndviImageUrl) {
    ...
    if (empty) {
      empty.textContent = ndviStatusMessage || "Run weekly_runner_v4 with a Google Earth Engine field asset to load NDVI.";
    }
  }
