...
Every day at 2:00 AM America/New_York, the workflow:

1. Installs the Python dependencies from `requirements.txt`.
2. Runs `python weekly_runner_v3.py --dashboard-only`.
...
Before the workflow can run, add these repository secrets:
...
The workflow also accepts the older secret name `API_TOKEN` as a fallback, but
`ZENTRA_API_TOKEN` is preferred.
...
For local runs, create a private `.env` file in this folder with:

```text
ZENTRA_API_TOKEN=Token your_token_here
```
