...
def zentra_fetch_end_date():
    ...
    return local_now().date().strftime("%Y-%m-%d")
...
def fetch_soil_moisture_openmeteo(
    history_start_date: str,
    history_end_date: str,
    forecast_start_date: str,
    forecast_end_date: str,
) -> list[dict]:
    """Fetch Open-Meteo soil moisture and aggregate hourly values by explicit dashboard dates."""
    hourly_vars = ["soil_moisture_3_9cm", "soil_moisture_27_81cm"]
    params = (
        f"latitude={LATITUDE}"
        f"&longitude={LONGITUDE}"
    ...
    url = f"https://api.open-meteo.com/v1/forecast?{params}"
    print(
        "\n  Fetching Open-Meteo soil moisture: "
        f"{history_start_date} → {history_end_date} history and "
        f"{forecast_start_date} → {forecast_end_date} forecast …"
    )
    ...
    shallow = hourly.get("soil_moisture_3_9cm", [])
    deep = hourly.get("soil_moisture_27_81cm", [])
    if not times:
        print("  ⚠  Open-Meteo soil moisture response did not include hourly timestamps.")
        return []
    ...
    for i, timestamp in enumerate(times):
        rows.append({
            ...
            "soilMoisture3To9": shallow[i] if i < len(shallow) else None,
            "soilMoisture27To81": deep[i] if i < len(deep) else None,
        })

    df = pd.DataFrame(rows)
    if df.empty:
        return []
    for col in ["soilMoisture3To9", "soilMoisture27To81"]:
        df[col] = pd.to_numeric(df[col], errors="coerce")

    daily = df.groupby("date", as_index=False)[["soilMoisture3To9", "soilMoisture27To81"]].mean()
    daily_dates = pd.to_datetime(daily["date"], errors="coerce").dt.date
    history_start = datetime.strptime(history_start_date, "%Y-%m-%d").date()
    history_end = datetime.strptime(history_end_date, "%Y-%m-%d").date()
    forecast_start = datetime.strptime(forecast_start_date, "%Y-%m-%d").date()
    forecast_end = datetime.strptime(forecast_end_date, "%Y-%m-%d").date()

    daily_lookup = daily.set_index("date")

    expected_history_dates = pd.date_range(history_start, history_end, freq="D").strftime("%Y-%m-%d").tolist()
    expected_forecast_dates = pd.date_range(forecast_start, forecast_end, freq="D").strftime("%Y-%m-%d").tolist()
    expected_rows = (
        [{"date": date, "type": "history"} for date in expected_history_dates]
        + [{"date": date, "type": "forecast"} for date in expected_forecast_dates]
    )

    result = []
    for expected in expected_rows:
        source = daily_lookup.loc[expected["date"]] if expected["date"] in daily_lookup.index else {}
        result.append({
            "date": expected["date"],
            "type": expected["type"],
            "soilMoisture3To9": round_or_none(source.get("soilMoisture3To9"), 3),
            "soilMoisture27To81": round_or_none(source.get("soilMoisture27To81"), 3),
        })
    print(f"  ✔  Soil moisture received ({len(result)} daily rows).")
    valid_shallow = [row["date"] for row in result if row["soilMoisture3To9"] is not None]
    valid_deep = [row["date"] for row in result if row["soilMoisture27To81"] is not None]
    if valid_shallow:
        print(f"     3-9 cm valid dates: {valid_shallow[0]} → {valid_shallow[-1]} ({len(valid_shallow)})")
    if valid_deep:
        print(f"     27-81 cm valid dates: {valid_deep[0]} → {valid_deep[-1]} ({len(valid_deep)})")
    return result
...
def initialize_earth_engine() -> tuple[bool, str]:
    ...
    try:
        if GEE_SERVICE_ACCOUNT_KEY_JSON:
            key_json = GEE_SERVICE_ACCOUNT_KEY_JSON.strip()
...
def build_ndvi_candidate(image, field) -> dict | None:
    ...
    mean_ndvi = stats_raw.get("NDVI_mean")
    if not total or mean_ndvi is None:
        print(f"  ⚠  Sentinel-2 scene {acquired_at} has no valid NDVI pixels over the field; trying older scene.")
        return None

    ndvi_stats = {
        "mean": round_or_none(mean_ndvi, 3),
        ...
        "lowVegetationPct": round_or_none((area_stats.get("low") or 0) / total * 100, 1),
        "moderateVegetationPct": round_or_none((area_stats.get("moderate") or 0) / total * 100, 1),
        "highVegetationPct": round_or_none((area_stats.get("high") or 0) / total * 100, 1),
    }
...
def fetch_latest_sentinel2_ndvi() -> dict:
    ...
    if ee is None:
        message = "earthengine-api is not installed; keeping existing Sentinel-2 NDVI."
        print(f"  ⚠  {message}")
        return {"preserveExisting": True, "statusMessage": message}
    ...
    try:
        ...
        if not image_count:
            ...
            print(f"  ⚠  {message}")
            return {"statusMessage": message}
        candidates = collection.toList(min(int(image_count), 20))
        selected = None
        for index in range(min(int(image_count), 20)):
            selected = build_ndvi_candidate(ee.Image(candidates.get(index)), field)
            if selected:
                break
        if not selected:
            message = "No recent Sentinel-2 scenes had valid clear NDVI pixels over this field."
            print(f"  ⚠  {message}")
            return {"statusMessage": message}

        rendered = selected["image"].visualize(
            min=-0.2,
            max=0.9,
        ...
        return {
            "imageUrl": f"{NDVI_IMAGE_FILENAME}?v={local_now().strftime('%Y%m%d%H%M%S')}",
            "acquiredAt": selected["acquiredAt"],
            "cloudPercent": round(float(selected["cloudPercent"]), 1) if selected["cloudPercent"] is not None else "",
...
def update_dashboard_ndvi_metadata(ndvi_meta: dict) -> None:
    ...
    if not app_js_path.exists():
        ...
        return
    if ndvi_meta.get("preserveExisting"):
        print("  ⚠  Keeping existing dashboard NDVI metadata.")
        return
...
def update_index_3_soil_moisture(soil_rows: list[dict]) -> None:
    project_dir = Path(__file__).resolve().parent
    index_path = project_dir / "index_3.html"
    if not index_path.exists():
        print(f"  ⚠  index_3.html not found; soil moisture charts were not updated: {index_path}")
        return

    html_text = index_path.read_text(encoding="utf-8")
    html_text = replace_js_const_array(html_text, "soilMoistureData", soil_rows)
    index_path.write_text(html_text, encoding="utf-8")
    print(f"  ✔  Soil moisture chart data updated → {index_path}")
...
def update_dashboard_app_js(
    ...
    js_text = replace_js_const_string(js_text, "dashboardUpdatedAt", local_now().strftime("%Y-%m-%d %H:%M"))
    app_js_path.write_text(js_text, encoding="utf-8")
...
def build_email_body(
    ...
    run_time = local_now().strftime("%Y-%m-%d %H:%M")
...
def main(dashboard_only: bool = False):
    ...
    print(f"  Run date: {local_now().strftime('%Y-%m-%d %H:%M')}\n")
    ...
    try:
        ...
        from datetime import datetime as dt, timedelta as td
        forecast_today = local_now().date()
        forecast_start = forecast_today.strftime("%Y-%m-%d")   # forecast from today
        second_week_forecast_start = (forecast_today + td(days=7)).strftime("%Y-%m-%d")
        soil_forecast_end = (forecast_today + td(days=6)).strftime("%Y-%m-%d")
        forecast_rows = fetch_forecast(forecast_start)
        second_week_forecast_rows = fetch_forecast(second_week_forecast_start)
        soil_moisture_rows = fetch_soil_moisture_openmeteo(
            start_date,
            end_date,
            forecast_start,
            soil_forecast_end,
        ...
        update_index_3_soil_moisture(soil_moisture_rows)
