...
def ndvi_interpretation(mean_ndvi: float | None, high_pct: float | None, low_pct: float | None) -> str:
    if mean_ndvi is None:
        return ""

    if mean_ndvi >= 0.6:
        condition = "strong overall vegetation vigor"
    elif mean_ndvi >= 0.4:
        condition = "moderate overall vegetation vigor"
    elif mean_ndvi >= 0.2:
        condition = "limited green vegetation or mixed bare soil and vegetation"
    else:
        condition = "mostly bare soil, water, shadow, or very stressed vegetation"

    details = []
    if high_pct is not None:
        details.append(f"{high_pct:.0f}% of valid pixels are high-NDVI vegetation")
    if low_pct is not None:
        details.append(f"{low_pct:.0f}% are low-NDVI/bare or stressed areas")

    suffix = f"; {', '.join(details)}." if details else "."
    return f"The latest Sentinel-2 scene indicates {condition}{suffix}"
...
def initialize_earth_engine() -> tuple[bool, str]:
    ...
    try:
        ...
        if not key_json and GEE_SERVICE_ACCOUNT_KEY_FILE:
            key_json = Path(GEE_SERVICE_ACCOUNT_KEY_FILE).read_text(encoding="utf-8")

        if key_json:
            key_data = json.loads(key_json)
            service_account = GEE_SERVICE_ACCOUNT or key_data.get("client_email") or DEFAULT_GEE_SERVICE_ACCOUNT
            if not service_account:
                raise ValueError("Service account email is missing from GEE credentials.")
            project = GEE_PROJECT or key_data.get("project_id")
            print(f"  Earth Engine service account: {service_account}")
            print(f"  Earth Engine project: {project or '(default)'}")
            ...
            credentials = ee.ServiceAccountCredentials(service_account, key_file=temp_key_path)
            ee.Initialize(credentials, project=project)
        else:
            print("  Earth Engine credentials: default local/application credentials")
            print(f"  Earth Engine project: {GEE_PROJECT or '(default)'}")
...
def fetch_latest_sentinel2_ndvi() -> dict:
    ...
    if not initialized:
        return ndvi_status(init_message)
    ...
    try:
        ...
        pixel_area = ee.Image.pixelArea().updateMask(ndvi.mask())
        ...
        class_areas = ee.Dictionary({
            "low": pixel_area.updateMask(ndvi.lt(0.3)).reduceRegion(
                reducer=ee.Reducer.sum(), geometry=field, scale=10, maxPixels=1e9
            ).get("area"),
            "moderate": pixel_area.updateMask(ndvi.gte(0.3).And(ndvi.lt(0.6))).reduceRegion(
                reducer=ee.Reducer.sum(), geometry=field, scale=10, maxPixels=1e9
            ).get("area"),
            "high": pixel_area.updateMask(ndvi.gte(0.6)).reduceRegion(
                reducer=ee.Reducer.sum(), geometry=field, scale=10, maxPixels=1e9
            ).get("area"),
        }).getInfo()
        total_area_value = ee.Number(total_area).getInfo()

        def pct(area):
            if not total_area_value or area is None:
                return None
            return round((float(area) / float(total_area_value)) * 100, 1)

        mean_ndvi = stats.get("NDVI_mean")
        min_ndvi = stats.get("NDVI_min")
        max_ndvi = stats.get("NDVI_max")
        std_ndvi = stats.get("NDVI_stdDev")
        high_pct = pct(class_areas.get("high"))
        moderate_pct = pct(class_areas.get("moderate"))
        low_pct = pct(class_areas.get("low"))
        ndvi_stats = {
            "mean": round(float(mean_ndvi), 3) if mean_ndvi is not None else "",
            "min": round(float(min_ndvi), 3) if min_ndvi is not None else "",
            "max": round(float(max_ndvi), 3) if max_ndvi is not None else "",
            "stdDev": round(float(std_ndvi), 3) if std_ndvi is not None else "",
            "highVegetationPct": high_pct if high_pct is not None else "",
            "moderateVegetationPct": moderate_pct if moderate_pct is not None else "",
            "lowVegetationPct": low_pct if low_pct is not None else "",
            "interpretation": ndvi_interpretation(
                float(mean_ndvi) if mean_ndvi is not None else None,
                high_pct,
                low_pct,
            ),
        }
...
def update_dashboard_ndvi_metadata(ndvi_meta: dict) -> None:
    ...
    if ndvi_meta.get("imageUrl"):
        ...
        js_text = replace_js_const_value(js_text, "ndviStats", {})
