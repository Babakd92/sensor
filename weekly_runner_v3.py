...
def fetch_soil_moisture_openmeteo(
    ...
    history = daily[(daily_dates >= history_start) & (daily_dates <= history_end)].copy()
    forecast = daily[(daily_dates >= forecast_start) & (daily_dates <= forecast_end)].copy()
    ...
    for _, row in combined.iterrows():
        result.append({
            "date": row["date"],
            "type": row["type"],
            "soilMoisture3To9": round_or_none(row["soilMoisture3To9"], 3),
            "soilMoisture27To81": round_or_none(row["soilMoisture27To81"], 3),
        })
