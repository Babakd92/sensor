...
def fetch_soil_moisture_openmeteo() -> list[dict]:
    """Fetch Open-Meteo soil moisture history/forecast and aggregate hourly values by day."""
    ...
    url = f"https://api.open-meteo.com/v1/forecast?{params}"
    print("\n  Fetching Open-Meteo soil moisture: 7-day history and 7-day forecast …")
    ...
    today = datetime.now().date()
    history = daily[pd.to_datetime(daily["date"]).dt.date < today].tail(7).copy()
    forecast = daily[pd.to_datetime(daily["date"]).dt.date >= today].head(7).copy()
